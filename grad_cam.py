...
# --- Global counter for sequential folder naming ---
GRADCAM_COUNTER = 1

def register_hooks_by_name(model: nn.Module, layer_name: str = "", save_all: bool = False):
    activations = {}
    gradients = {}
    handles = []
    ...
    if save_all:
        ...
        exact_match = [(name, layer) for name, layer in named_layers if name == layer_name]
        if exact_match:
            name, layer = exact_match[0]
        else:
            matching = [(name, layer) for name, layer in named_layers if name.startswith(layer_name)]
            if not matching:
            ...
            if len(matching) > 1:
                ...
                raise ValueError(f"Multiple layers found starting with '{layer_name}': {available}. Please specify exact layer name.")
...
def grad_cam(model, image: torch.Tensor, layer_name: str = None, 
             save_all: bool = False, output_dir: str = "cam_outputs", 
             print_shapes: bool = True, is_lstm_model: bool = False):
    """
    ...
    if save_all and layer_name is not None:
        raise ValueError("Cannot use `save_all=True` and `layer_name` at the same time.")
    ...
    activations, gradients, handles = register_hooks_by_name(model, layer_name, save_all)
    logits = model(image) 
    predicted_class_idx = logits.argmax(dim=1).item()
    model.zero_grad()
    logits[0, predicted_class_idx].backward()

    def compute_and_save_cam(activation: torch.Tensor, gradient: torch.Tensor, name: str,
                             image: Optional[torch.Tensor] = None,
                             base_save_dir: str = None, print_shapes: bool = False):
        global GRADCAM_COUNTER

        # Ստեղծում ենք նոր պապկա յուրաքանչյուր պատկերի համար
        folder_name = os.path.join(base_save_dir, f"cam_{GRADCAM_COUNTER}")
        os.makedirs(folder_name, exist_ok=True)

        if print_shapes:
            print(f"[{name}] Activation: {activation.shape}, Gradient: {gradient.shape}")
        ...
        if activation.shape[2] <= 2 or activation.shape[3] <= 2:
            print(f"Skipping layer {name} - spatial dims too small ({activation.shape[2]}x{activation.shape[3]}).")
            return
        ...
        if image is not None:
            ...
            cv2.imwrite(os.path.join(folder_name, f"gradcam_{GRADCAM_COUNTER}.jpg"), superimposed)
            plt.imsave(os.path.join(folder_name, f"original_{GRADCAM_COUNTER}.jpg"), image_np)
            plt.imsave(os.path.join(folder_name, f"heatmap_plot_{GRADCAM_COUNTER}.jpg"), heatmap_np)
    ...
    if save_all:
        for name in activations:
            ...
            if gradient is not None:
                compute_and_save_cam(activation, gradient, name, image, base_save_dir=output_dir, print_shapes=print_shapes)
            else:
        ...
        if gradient is not None:
            compute_and_save_cam(activation, gradient, layer_name, image, base_save_dir=output_dir, print_shapes=print_shapes)
        else:
